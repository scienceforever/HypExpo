...
HypExpo <- function(df, param.hyp=1, param.expo=1.8, param.diag=0, param.shift=0.17, baseName="test", preview.only=TRUE) {              
theCurve <- function(x) param.hyp / log10(x)^param.expo - param.diag * log10(x) + param.shift
paramChain <- paste(param.hyp, param.expo, param.diag, param.shift, sep="_")
imgName <- paste(baseName, paramChain, "png", sep=".")
df <- cbind(df, passed = as.numeric(log10(df[,2]) >= theCurve(df[,1])))
df.sel <- df[df[,ncol(df)]==1,] # column number may differ depending on the number of selection runs with different parameters
...
curve(param.hyp / x^param.expo - param.diag * x + param.shift, add=TRUE, col="green") }
#
if (!(preview.only)) {
